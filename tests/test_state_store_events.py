...
Tests async methods with IsolatedAsyncioTestCase.
"""
...
class TestDetectionEventsTable(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_record_detection_creates_row(self):
        """record_detection should insert a row with mitigated_at=None."""
        row_id = await self.store.record_detection(
            prefix="192.0.2.0/24", class_id=3, trust_score=0.10
        )
        self.assertIsNotNone(row_id)
        self.assertGreater(row_id, 0)

        row = await self.store.get_latest_detection("192.0.2.0/24")
    ...
    async def test_record_detection_detected_at_is_recent(self):
        """detected_at should be close to the current epoch time."""
        before = time.time()
        await self.store.record_detection("192.0.2.0/24", class_id=1, trust_score=0.55)
        after = time.time()
        row = await self.store.get_latest_detection("192.0.2.0/24")
    ...
    async def test_record_mitigation_fills_timestamp(self):
        """record_mitigation should set mitigated_at on the open row."""
        await self.store.record_detection("192.0.2.0/24", class_id=3, trust_score=0.05)
        updated = await self.store.record_mitigation("192.0.2.0/24")
    ...
    async def test_record_mitigation_returns_false_with_no_open_row(self):
        """record_mitigation should return False when no open detection exists."""
        updated = await self.store.record_mitigation("10.0.0.0/8")
        self.assertFalse(updated)

    async def test_record_mitigation_only_updates_open_row(self):
        """After mitigation, calling record_mitigation again should return False."""
        await self.store.record_detection("192.0.2.0/24", class_id=3, trust_score=0.05)
        await self.store.record_mitigation("192.0.2.0/24")  # closes the row
        updated_again = await self.store.record_mitigation("192.0.2.0/24")
    ...
    async def test_get_latest_detection_returns_most_recent(self):
        """With multiple rows, get_latest_detection returns the newest."""
        await self.store.record_detection("192.0.2.0/24", class_id=1, trust_score=0.60)
        time.sleep(0.01)  # ensure distinct timestamps
        await self.store.record_detection("192.0.2.0/24", class_id=3, trust_score=0.10)
        row = await self.store.get_latest_detection("192.0.2.0/24")
        self.assertEqual(row["class_id"], 3,
                         msg="Should return the most recent row (class_id=3)")

    async def test_get_latest_detection_returns_none_for_unknown_prefix(self):
        """get_latest_detection should return None for an unrecorded prefix."""
        row = await self.store.get_latest_detection("1.2.3.4/32")
    ...
    async def test_detection_events_independent_of_active_policies(self):
        """detection_events and active_policies are independent tables."""
        await self.store.save_policy(
